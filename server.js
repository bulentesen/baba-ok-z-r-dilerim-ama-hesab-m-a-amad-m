...
<body>
  <div class="wrap">
    <div class="card">
      <div class="top">
        <div>
          <span class="pill">Mini Chat</span>
  ...
  <script>
    ...
    socket.on("system", (m) => {
      addLine('<span class="sys">' + esc(m.text) + '</span>');
    });

    socket.on("presence", ({ members }) => {
      elMembers.textContent = "online: " + members.length + " | " + members.join(", ");
    });

    socket.on("typing", ({ name, isTyping }) => {
      elTyping.textContent = isTyping ? (name + " yazıyor...") : "";
    });
...
io.on("connection", (socket) => {
  socket.on("join", ({ name, room }) => {
    ...
    const members = [...onlineUsers.values()]
      .filter(u => u.room === safeRoom)
      .map(u => u.name);
  ...
  socket.on("chat", ({ room, name, text }) => {
    ...
    io.to(safeRoom).emit("chat", { name: safeName, text: msg, ts: Date.now() });
  });
